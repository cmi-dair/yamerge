...
def _cli_parse_args():
    parser = argparse.ArgumentParser(
        description="Preprocessor for YAML files. Merge data from other YAML files "
...
def main():
    ...
    yaml_data = TransformerSystem(
        [
            AbsoluteMergeTransformerGenerator(
                merge_tag_name="MERGE", paths=search_paths
            ),
            RelativeMergeTransformerGenerator(
                merge_tag_name="MERGE-SNIPPET", paths=search_paths
            ),
        ]
    ).apply(yaml_data)
